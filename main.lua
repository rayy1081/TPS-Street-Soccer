-- [[ 🛡️ ANTIBAN & ANTI-CHEAT BYPASS PROTOKOLÜ ]]
local Players = game:GetService("Players")
local LocalPlayer = Players.LocalPlayer
local HookingSuccess, HookError = pcall(function()
    local X7_Meta = getrawmetatable(game)
    local Old_Index = X7_Meta.__index
    setreadonly(X7_Meta, false)

    X7_Meta.__index = newcclosure(function(self, property)
        if _G.ReachEnabled and (self.Name == "Football" or self.Name == "Ball") and property == "Size" then
            return Vector3.new(3, 3, 3)
        end
        return Old_Index(self, property)
    end)
    setreadonly(X7_Meta, true)
end)

-- ADMIN DEDEKTÖRÜ
Players.PlayerAdded:Connect(function(player)
    task.wait(1)
    if player:GetRankInGroup(4367320) >= 100 or player.Name:lower():find("mod") or player.Name:lower():find("admin") then
        LocalPlayer:Kick("[TPS HUB ANTIBAN] Sunucuya yetkili bir personel katildi! Güvenliğiniz için oyundan atildiniz.")
    end
end)

-- [[ ANA HİLE PANELİ BAŞLANGICI ]]
...
-- Global Ayarlar (Reach başlangıçta 5 olarak ayarlı)
...
RunService.Heartbeat:Connect(function()
    ...
    if _G.ReachEnabled then
        ...
        if ball.Size.X ~= 3 then ball.Size = Vector3.new(3, 3, 3) end
    end
    ...
    if _G.AirDribble then
        ...
        if ball:FindFirstChild("AirVelocity") then ball.AirVelocity:Destroy() end
    end
...
local function StealAvatar(targetName)
    ...
    if targetPlayer and targetPlayer.Character and LocalPlayer.Character then
        for _, v in pairs(LocalPlayer.Character:GetChildren()) do
            if v:IsA("Accessory") or v:IsA("Shirt") or v:IsA("Pants") or v:IsA("BodyColors") then v:Destroy() end
        end
...
TopBar.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
        ...
        input.Changed:Connect(function() if input.UserInputState == Enum.UserInputState.End then mainDragging = false end end)
    end
end)
TopBar.InputChanged:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch then mainDragInput = input end end)
UserInputService.InputChanged:Connect(function(input) if input == mainDragInput and mainDragging then updateMain(input) end end)
...
-- COMBAT SAYFASI
CreateToggle(CombatPage, "Reach Arttirma", function(state) _G.ReachEnabled = state end)

-- Dinamik Reach Boyut Ayarlayıcı TextBox
CreateTextBox(CombatPage, "Reach Size (Current: 5 - Press Enter)", function(text)
    local num = tonumber(text)
    if num then
        _G.ReachSize = num
        print("Reach boyutu güncellendi: " .. tostring(num))
    end
end)

CreateToggle(CombatPage, "Alz React Mode", function(state) ApplyReactPreset(state and "Alz" or "None") end)
CreateToggle(CombatPage, "Abz React Mode", function(state) ApplyReactPreset(state and "Abz" or "None") end)
CreateToggle(CombatPage, "Tunaz React Mode", function(state) ApplyReactPreset(state and "Tunaz" or "None") end)
CreateToggle(CombatPage, "Azrael React Mode", function(state) ApplyReactPreset(state and "Azrael" or "None") end)

-- SKILLS SAYFASI
CreateToggle(SkillsPage, "Air Dribble Helper", function(state) _G.AirDribble = state end)
CreateToggle(SkillsPage, "Inf Dribble Helper", function(state) _G.InfDribble = state end)

-- MISC SAYFASI
CreateTextBox(MiscPage, "Target Username (Press Enter)", function(text) StealAvatar(text) end)
CreateToggle(MiscPage, "FPS Booster", function(state) if state then DynamicFPSBooster() end end)
RunService.Heartbeat:Connect(function()
    local ball = GetBall()
    if not ball or not LocalPlayer.Character or not LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then return end
